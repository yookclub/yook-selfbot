...
ctypes.windll.kernel32.SetConsoleTitleW(f'[Yooks Selfbot v{1.0}] | Loading...')
...
def Init():
    if config.get('token') == "token-here":
        ...
        token = config.get('token')
        try:
            yook.run(token, bot=False, reconnect=True)
            os.system(f'title (Yooks Selfbot) - Version {SELFBOT.__version__}')
...
def GmailBomber():
    ...
    counter = eval(input('Ammount of times to send: '))
    count = 0
    while count < counter:
        count = 0
...
yook = discord.Client()
yook = commands.Bot(
    description='yook Selfbot',
    command_prefix=prefix,
    self_bot=True
)
yook.remove_command('help') 
...
async def btc_status():
    ...
    btc_stream = discord.Streaming(
        ...
        url="https://www.twitch.tv/bnbu", 
    )
    await yook.change_presence(activity=btc_stream)
...
async def on_message_edit(before, after):
    await yook.process_commands(after)
...
async def on_message(message):
    ...
    if f'Congratulations <@{yook.user.id}>' in message.content:
        if giveaway_sniper == True:
            if message.author.id == 294882584201003009:    
                print(""
    ...
    if 'privnote.com' in message.content:
        if privnote_sniper == True:
            ...
            return
    await yook.process_commands(message)
...
async def on_connect():
    ...
    startprint()
    ctypes.windll.kernel32.SetConsoleTitleW(f'[yook Selfbot v{SELFBOT.__version__}] | Logged in as {yook.user.name}')
                      ...
                      module.prototype._connect = function(url) {
                        ...
                        this._handleDispatch = function(data, type) {
                          if (type === 'READY') {
                            console.log(data);
                            data.user.bot = false;
                            data.user.email = 'horsefuck@example.com';
...
async def weather(ctx, *, city): # b'\xfc'
    await ctx.message.delete()
    if weather_key == '':
        print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}Weather API key has not been set in the config file retard."+Fore.RESET)
    else:
        try:
...
@yook.command(aliases=['shorteen'])
async def bitly(ctx, *, link): # b'\xfc'
    await ctx.message.delete()
    if bitly_key == '':
        print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}Bitly API key has not been set in the config file retard."+Fore.RESET)
    else:
        try:
...
async def cuttly(ctx, *, link): # b'\xfc'
    await ctx.message.delete()
    if cuttly_key == '':
        print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}Cutt.ly API key has not been set in the config file retard."+Fore.RESET)
    else:
        try:
...
async def cat(ctx): # b'\xfc'
    await ctx.message.delete()
    if cat_key == '':
        print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}Cat API key has not been set in the config file retard."+Fore.RESET)
    else:
        try:
...
@yook.command(name='ebay-view', aliases=['ebay-view-bot', 'ebayviewbot', 'ebayview'])
...
@yook.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
...
@yook.command(aliases=['pfp', 'avatar'])
...
@yook.command(name='1337-speak', aliases=['1337speak'])
...
async def blank(ctx): # b'\xfc'
    ...
    if config.get('password') == 'password-here':
        ...
        with open('Images/Avatars/Transparent.png', 'rb') as f:
          try:      
             await yook.user.edit(password=password, username="ٴٴٴٴ", avatar=f.read())
          except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@yook.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
              await yook.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@yook.command(name='set-pfp', aliases=['setpfp', 'pfpset'])
async def _set_pfp(ctx, *, url): # b'\xfc'
    await ctx.message.delete()
    if config.get('password') == 'password-here':
        print(f"{Fore.RED}[ERROR] {Fore.YELLOW}You didnt put your password in the config file retard"+Fore.RESET)
    else:
        password = config.get('password')
        with open('Images/Avatars/profileAES.png', 'wb') as f:
          ...
          for block in r.iter_content(1024):
              ...
              f.write(block)
    try:
        Image.open('Images/Avatars/profileAES.png'   ).convert('RGB')
        with open('Images/Avatars/profileAES.png', 'rb') as f:
            await yook.user.edit(password=password, avatar=f.read())
    except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@yook.command(aliases=['dong', 'penis', 'cock'])
async def dick(ctx, *, user: discord.Member = None): # b'\xfc'
    ...
    for _i in range(0, size):
        dong += "="
    em = discord.Embed(title=f"{user}'s pp size", description=f"8{dong}D", colour=0x0000)
    await ctx.send(embed=em)

@yook.command(aliases=['changehypesquad'])
...
@yook.command(aliases=['tokenfucker', 'disable', 'crash']) 
...
@yook.command(aliases=['fakeconnection', 'spoofconnection'])
...
@yook.command(aliases=['tokinfo', 'tdox'])
...
async def copy(ctx): # b'\xfc'
    await ctx.message.delete()
    await yook.create_guild(f'backup-{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in yook.guilds:
...
async def destroy(ctx): # b'\xfc'
    ...
    try:
        await ctx.guild.edit(
            name=RandString(),
            description="https://yook.club",
            reason="https://yook-selfbot.github.io",
            icon=None,
            banner=None
        )  
    except:
...
async def dm(ctx, user : discord.Member, *, message): # b'\xfc'
    await ctx.message.delete()
    user = yook.get_user(user.id)
    if ctx.author.id == yook.user.id:
        return
    else:
        try:
            await user.send(message) 
...
@yook.command(name='get-color', aliases=['color', 'colour', 'sc'])
...
@yook.command(aliases=['rainbow-role'])
...
@yook.command(aliases=['slots', 'bet'])
...
@yook.command(name='auto-bump', aliases=['bump'])
async def _auto_bump(ctx, channelid): # b'\xfc'
    ...
    while True:
        try:
            count += 1 
            channel = yook.get_channel(int(channelid))
...
@yook.command(name='backup-f', aliases=['friendbackup', 'friend-backup', 'backup-friends', 'backupf'])
async def _backup_f(ctx): # b'\xfc'
    await ctx.message.delete()
    for friend in yook.user.friends:
       friendlist = (friend.name)+'#'+(friend.discriminator)   
       with open('Backup/Friends.txt', 'a+') as f:
           f.write(friendlist+"\n" )
    for block in yook.user.blocked:
        blocklist = (block.name)+'#'+(block.discriminator)
        with open('Backup/Blocked.txt', 'a+') as f: 
            f.write(blocklist+"\n" )

@yook.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
...
@yook.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
async def purge(ctx, amount: int): # b'\xfc'
    await ctx.message.delete()
    async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == yook.user).map(lambda m: m):
        try:
           await message.delete()
        except:
            pass

@yook.command(name='group-leaver', aliase=['leaveallgroups', 'leavegroup', 'leavegroups'])
async def _group_leaver(ctx): # b'\xfc'
    await ctx.message.delete()
    for channel in yook.private_channels:
...
async def help(ctx): # b'\xfc'
    await ctx.message.delete()
    url = 'https://yook-selfbot.github.io/commands'
    r = requests.get(url)
    if r.status_code == 200:
        webbrowser.open(url)
    else:
        print('Github has been taken down, either that or your internet is dogshit.')    
...
async def stream(ctx, *, message): # b'\xfc'
    ...
    await yook.change_presence(activity=stream)    
...
async def game(ctx, *, message): # b'\xfc'
    ...
    await yook.change_presence(activity=game)

@yook.command()
async def listening(ctx, *, message): # b'\xfc'
    await ctx.message.delete()
    await yook.change_presence(
...
@yook.command(aliases=['markasread', 'ack'])
async def read(ctx): # b'\xfc'
    await ctx.message.delete()
    for guild in yook.guilds:
        await guild.ack()
...
@yook.command(name='role-hexcode', aliases=['rolecolor'])
async def _role_hexcode(ctx, *, role: discord.Role): # b'\xfc'
    await ctx.message.delete()
    await ctx.send(f"{role.name} : {role.color}")

@yook.command(name='get-hwid', aliases=['hwid', 'gethwid', 'hwidget'])
...
@yook.command(aliases=['btc-stream', 'streambtc', 'stream-btc', 'btcstatus'])
async def btcstream(ctx):  # b'\xfc'
    await ctx.message.delete()   
    btc_status.start()        

@yook.command(name='steal-all-pfp', aliases=['steal-all-pfps', 'stealallpfps'])
async def _steal_all_pfp(ctx): # b'\xfc'
    await ctx.message.delete()
    Dump(ctx)

@yook.command(aliases=['clearconsole', 'consoleclear'])
...
@yook.command(name='gmail-bomb', aliases=['gmail-bomber', 'gmailbomb', 'email-bomber', 'emailbomber'])
